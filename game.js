...
function checkBoard() {
    console.log("stan tablicy: " + state);
    active_pill = 0;
    startBoard();
    drawArrow();
    ...
    for (let i = 0; i < 5; i++) { 
        ...
        if (state[i] == solution[i]) {
            ...
            let diff_place = false;
            for (let j = 0; j <5; j++) {
                if (state[i] == solution[j]) diff_place = true;
            }
    ...
    if (win) {
        ...
        document.querySelector(".comment").innerHTML ="You've managed to guess all the colors in round number " + round_no;
        ...
        document.querySelector("#again").style.color = "pink";
...
function drawArrow() {
    ...
    ctx.fillStyle = "white";
    ctx.font = "32px Arial";
    ctx.fillText("↑", 130 + active_pill * 100, 180);
